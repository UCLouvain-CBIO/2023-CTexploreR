...
% of CTdatabase in CTexploreR, which is `r length(CTdatabase_ours@IntersectionSets[["11"]])/nrow(CT_genes)*100`
% of our database.
...
`r (nrow(CTdatabase_lost) - table(CTdatabase_lost$testis_specificity)[["testis_specific"]])/nrow(CTdatabase_lost)*100`% of 
...
`r sum(table(CT_genes$testis_cell_type)[c("Early_spermatocyte", "Spermatogonia", "SSC")])/length(genes_avail)*100`
...
These genes are on the X chromosome but escape X chromosome inactivation and are
