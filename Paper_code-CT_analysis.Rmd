...
CT_genes_types <- CT_genes %>% 
  ...
  mutate(specificity = case_when(GTEX_category == "testis_specific" ~ "Testis specific",
...
Chr_colors = c("X-linked" = "deeppink", "not X" = "lightgreen")
...
Fetal_cell_colors = c( "F_PGC" = "pink", 
                       ...
                       "M_PGC" = "lightblue1", 
                       "M_GC" = "steelblue3",
                       "M_pre_spermatogonia" = "darkblue")

oocytes_colors <- c("Growing oocytes" = "paleturquoise3", 
                  # "Fully grown oocytes" ="lightgreen", 
                  # "Metaphase I" = "mediumvioletred", 
                  "Metaphase II" = "mediumvioletred")

sex_colors <- c("F" = "palevioletred1", "M"= "skyblue2")

stage_colors <- c( "pre-meiotic" = "moccasin", 
                   "meiotic"= "lightsalmon",
...
mean_methylation_fig <- function(genes = NULL, 
  ...
  h <- Heatmap(mat[gene_type$gene, sort(colnames(mat))],
               ...
               border = TRUE,
               border_gp = gpar(lwd = 0.5),
               row_title_gp = gpar(col = c("indianred2", "cyan4"), fontsize = 6),
...
DAC_fig <- function(genes = gene_type$gene, multimapping = TRUE, 
  ...
  column_ha_treatment <- HeatmapAnnotation(
    ...
    col = list(treatment = c("CTL" = "royalblue4", "5-aza" = "maroon3")),
    annotation_legend_param = legends_param,
    border = FALSE, 
    simple_anno_size = unit(0.2, "cm"),
    annotation_name_gp = gpar(fontsize = 0))  
...
fig_X_enrichment <- ggplot(CT_genes_types, aes(x = chr, fill = Regulation)) +
  geom_bar(stat = 'count') +
  xlab("Chromosome") +
  ylab("Gene number") +
...
fig_freq_by_chr <- ggplot(CT_genes_types, aes(x = chr, fill = Regulation)) +
  geom_bar(stat = 'count') +
  xlab("Chromosome") +
  ylab("Gene number") +
  scale_fill_manual(values = Regulation_colors) +
...
fig_mean_prop_in_tum <- ggplot(mean_and_prop,
...
print_TCGA_heatmap_with_AUCscores <- function(genes = NULL,
  ...
  h <- Heatmap(mat[gene_type$gene, rownames(df_col)],
               ...
               column_title_gp = gpar(col =c("COAD" = "darkorchid2", 
                                             "HNSC" = "deeppink2",
                                             "LUSC" = "seagreen3", 
                                             "SKCM" = "red3"), 
...
h1 <- print_TCGA_heatmap_with_AUCscores(genes = met_genes, 
                                        ...
                                        fontsize = 1, 
                                        scale_lims = c(0, 6),
                                        # tum_type = c("SKCM", "LUAD", "COAD", "BRCA"),
                                        tum_type = tum_types,
                                        split_rows_by = NULL)
...
draw(h1, column_title = "CT genes regulated by methylation",
     column_title_gp = gpar(fontsize = 5), show_heatmap_legend = FALSE,
...
f1 <- as_tibble(colData(TCGA), rownames = "barcode") %>% 
  ...
  ggplot(aes(x = global_methylation, y = AUC_genes_regulated_by_met, color = project_id)) +
  geom_point(size = 0.3) +
  facet_wrap(~ project_id, nrow = 1) +
  scale_color_manual(values = c("COAD" = "darkorchid2", "HNSC" = "deeppink2",
                                "LUSC" = "seagreen3", "SKCM" = "red3")) + 
...
f2 <- as_tibble(colData(TCGA), rownames = "barcode") %>% 
  filter(project_id %in% tum_types) %>% 
  mutate(project_id = factor(project_id, levels = tum_types,)) %>% 
  ggplot(aes(x = global_methylation, y = AUC_genes_not_regulated_by_met, color = project_id)) +
  geom_point(size = 0.3) +
  facet_wrap(~ project_id, nrow = 1) +
  scale_color_manual(values = c("COAD" = "darkorchid2", "HNSC" = "deeppink2",
                                "LUSC" = "seagreen3", "SKCM" = "red3")) + 
...
testis_sce[, testis_sce$type %in% 
             c("Early_spermatocyte", "Late_spermatocyte", 
               "Round_spermatid")]$stage <- "meiotic"
testis_sce[, testis_sce$type %in% 
             c("Elongated_spermatid", "Sperm")]$stage <- "post-meiotic"
testis_sce[, testis_sce$type %in% 
...
draw_oocytes_heatmap <- function(genes, 
  ...
  column_ha_cell_type = HeatmapAnnotation(
    ...
    col = list(Adult_cell_type = oocytes_colors),      
    border = TRUE,
    simple_anno_size = unit(0.15, "cm"),
    annotation_name_gp = gpar(fontsize = 0),
    annotation_legend_param = legends_param_sc)
...
draw(ht_list, main_heatmap = "logcounts", merge_legend = TRUE, ht_gap = unit(0.2, "cm"))
